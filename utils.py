...
class NSGA2Utils:
    ...
    def create_initial_population_new(self):
        ...
        population = Population()
        count=0
        constrained_coverage = self.problem.demand_num* self.problem.coverage_constrain
        while count < self.num_of_individuals:
            individual = self.problem.generate_individual()
            self.problem.calculate_objectives(individual)
            if np.abs(individual.objectives[0])>constrained_coverage:
                population.append(individual)
                count+=1
        return population
    ...
    def create_children(self, population):
        ...
        while len(children) < len(population):  # 得到一个和population一样大小的children的解
            ...
            # check and fix station upbound
            station_upbound = self.problem.station_upbound


            pos_num1 = np.sum(child1.chromosome)
            if pos_num1>station_upbound:
                child1.upbound_constraint(station_upbound, pos_num1)

            pos_num2 = np.sum(child2.chromosome)
            if pos_num2 > station_upbound:
                child2.upbound_constraint(station_upbound, pos_num2)
          
            self.problem.calculate_objectives(child1)
            self.problem.calculate_objectives(child2)
            
            # coverage lowerbound
            if np.abs(child1.objectives[0])>constrained_coverage:
             children.append(child1)

            # if np.abs(child2.objectives[1])>0.5*len(child1.chromosome):
            if np.abs(child2.objectives[0])> constrained_coverage:
    ...
    def _crossover(self, individual1, individual2):
        ...
        temp = child1.chromosome[p:].copy() 
        child1.chromosome[p:] = child2.chromosome[p:]
        child2.chromosome[p:] = temp
        #child1.chromosome[p:] = child2.chromosome[p:]
